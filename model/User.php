...
class User extends \DenDad\base\BaseModel implements \DenDad\interfaces\IModel {
    ...
    function __construct($id = null) {
        parent::__construct($id, 'login', FALSE, array(
            'login',
            'password',
            'salt',
            'den',
    ...
    public function validate() {
        ...
        foreach ($this->_columnMap as $col) {
            switch ($col) {
                case ('email'):
                    if (!\DenDad\libs\Validation::validateEmailAddress(
                            $this->get($col)
                    ) || \trim($this->get($col)) == '') {
                        $invalid[] = $col;
                    }
